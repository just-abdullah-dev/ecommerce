...
export function AllSubAccounts({ allowedSubAccounts, setCurrentSubAccounts }) {
  ...
  const fetchItems = async () => {
    try {
      setIsLoading(true);
      const response = await fetchCustom("/user/sub-accounts", {
        method: "GET",
        token: user?.token,
      });
      ...
      if (data?.success) {
        ...
        setCurrentSubAccounts(data?.data.length)
      } else {
  ...
  // Update filteredItems whenever subAccounts, searchTerm, or roleFilter change
  useEffect(() => {
    const newFilteredItems = subAccounts.filter((employee) => {
      const fullName = `${[
  ...
  }, [subAccounts, searchTerm, roleFilter]);
    ...
    <Card className="w-full">
      <CardContent>
        ...
        <div className=" flex items-center gap-6">
          <div>Current Sub Accounts: {filteredItems.length}</div>
          <div>Total Sub Accounts in Plan: {allowedSubAccounts}</div>
        </div>
        {isError ?
                ...
                <TableRow key={employee?._id}>
                  ...
                  <TableCell className="flex items-center gap-4">
                    <Button
                      ...
                      variant="ghost"
                      size="icon"
                      onClick={() => setEditSubAccount(employee)}
                    >
                      <Edit3 size={20} />
                    </Button>
                  </TableCell>
                  <TableCell>
                    <ConfirmationDialog
                      ...
                      title="Are you sure?"
                      description="SubAccount will be removed."
                      onConfirm={() => handleDeactivate(employee?._id)}
                    />
                  </TableCell>
                </TableRow>
